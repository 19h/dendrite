...
| Transfer | streaming multi-source discovery, inbound peer promotion, contribution-aware choking, rarest-first, up to 512 peers per torrent, rate-sized request pipelines that span pieces, endgame, seeding; HTTP(S) web seeds | no per-torrent destination, sequential mode, ratio target, or bandwidth policy in API v2.0; web seeds reject non-public addresses |
